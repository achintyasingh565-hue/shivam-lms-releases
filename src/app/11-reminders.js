  ...
  function saveTemplates(){
    TPL={ reminder:$('tpl_reminder').value, overdue:$('tpl_overdue').value, greeting:$('tpl_greeting').value, holiday:$('tpl_holiday').value,
  ...
  function renderGreetings(){
    ...
    const GR_CAT={greeting:'Greeting / Notice',holiday:'Office Closure',thanks:'Payment Confirmation',welcome:'Loan Approval',finalnotice:'Final Notice',demandnotice:'Default Notice',bounce:'Cheque Bounce',closed:'Loan Closed',cleared:'Cheque Cleared'}; const gcat=GR_CAT[type]||'Greeting / Notice'; const grVars=(l)=>{ if(type==='holiday') return {name:l.name, occasion:occ||'', date:dt||''}; if(type==='thanks') return {name:l.name, amount:inrPlain(l.emi), acno:l.acno||'', outstanding:inrPlain(l.outstanding)}; if(type==='welcome') return {name:l.name, amount:inrPlain(l.principal), acno:l.acno||'', emi:inrPlain(l.emi)}; if(type==='finalnotice') return {name:l.name, outstanding:inrPlain(l.outstanding), acno:l.acno||''}; if(type==='demandnotice') return {name:l.name, acno:l.acno||'', arrears:inrPlain(l.arrears||0), due_date:(fmtDate(l.due)||''), outstanding:inrPlain(l.outstanding)}; if(type==='bounce'){ var bc=(l.charges||[]).filter(function(c){return c.type==='Cheque bounce';})[0]||{}; return {name:l.name, cheque:bc.cheque||'', acno:l.acno||'', amount:inrPlain(bc.amount||0)}; } if(type==='closed'){ return {name:l.name, acno:l.acno||'', principal:inrPlain(l.principal||0)}; } if(type==='cleared'){ var cc=(l.payments||[]).filter(function(p){return p.mode==='Cheque'&&p.status==='Cleared';}).slice(-1)[0]||{}; return {name:l.name, acno:l.acno||'', cheque:cc.cheque||'', amount:inrPlain(cc.amount||0)}; } return {name:l.name, occasion:occ||''}; }; window._grList = list.map(l=>({lang:lang, name:l.name, phone:l.phone, acno:l.acno, cat:gcat, msg:applyVars(tpl,l,Object.assign({occasion:occ||'',date:dt||''}, (['bounce','closed','cleared'].indexOf(type)>=0)?grVars(l):{})), tpl:tpl, loanId:l.id, amt:(/\{(emi|amount|outstanding)\}/.test(tpl)?Math.round(Number(String((grVars(l).amount)||'').replace(/[^0-9.]/g,''))||(Number(l.emi)||0)):null), vars:grVars(l)}));
