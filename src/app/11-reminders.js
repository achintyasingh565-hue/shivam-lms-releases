  ...
  function renderGreetings(){
    ...
    else if(type==='chequepresent'){ list=list.filter(l=>Array.isArray(l.payments)&&l.payments.some(p=>p.mode==='Cheque'&&p.status!=='Cleared')); }
    ...
    const GR_CAT={greeting:'Greeting / Notice',birthday:'Birthday Greeting',holiday:'Office Closure',thanks:'Payment Confirmation',welcome:'Loan Approval',finalnotice:'Final Notice',demandnotice:'Default Notice',chequepresent:'Cheque Presentation',bounce:'Cheque Bounce',closed:'Loan Closed',cleared:'Cheque Cleared',restructure:'Loan Restructured',accwelcome:'Welcome / Account Opened',launch:'Business Launched'}; const gcat=GR_CAT[type]||'Greeting / Notice'; const grVars=(l)=>{ var _lp=(l.payments||[]).filter(function(p){return Number(p.amount)>0;}).slice(-1)[0]||null; var _cc=(l.payments||[]).filter(function(p){return p.mode==='Cheque'&&p.status==='Cleared';}).slice(-1)[0]||{}; var _pc=(l.payments||[]).filter(function(p){return p.mode==='Cheque'&&p.status!=='Cleared';}).slice(-1)[0]||{}; var _bc=(l.charges||[]).filter(function(c){return c.type==='Cheque bounce';})[0]||{}; var _due=(fmtDate(l.due)||fmtDate(l.disb)||''); if(type==='holiday') return {name:l.name, occasion:occ||'', date:dt||''}; if(type==='thanks') return {name:l.name, amount:inrPlain(_lp?_lp.amount:l.emi), acno:l.acno||'', txn:((_lp&&(_lp.ref||_lp.cheque||fmtDate(_lp.date)))||_due||('A/C '+(l.acno||''))), outstanding:inrPlain(l.outstanding)}; if(type==='welcome') return {name:l.name, acno:l.acno||'', emi:inrPlain(l.emi), due_date:_due, amount:inrPlain(l.principal)}; if(type==='finalnotice') return {name:l.name, acno:l.acno||'', outstanding:inrPlain(l.outstanding), due_date:_due}; if(type==='demandnotice') return {name:l.name, acno:l.acno||'', arrears:inrPlain(l.arrears||0), due_date:_due, outstanding:inrPlain(l.outstanding)}; if(type==='bounce') return {name:l.name, acno:l.acno||'', amount:inrPlain(_bc.amount||0), txn:(_bc.cheque||''), cheque:(_bc.cheque||'')}; if(type==='closed') return {name:l.name, acno:l.acno||'', principal:inrPlain(l.principal||0), disbursed:inrPlain(l.principal||0)}; if(type==='cleared') return {name:l.name, acno:l.acno||'', amount:inrPlain(_cc.amount||0), txn:(_cc.cheque||''), cheque:(_cc.cheque||'')}; if(type==='chequepresent') return {name:l.name, acno:l.acno||'', amount:inrPlain(_pc.amount||l.emi||0), cheque:(_pc.cheque||''), bank:(_pc.bank||'your bank'), date:(dt||fmtDate(_pc.date)||_due||'')}; if(type==='restructure') return {name:l.name, acno:l.acno||'', emi:inrPlain(l.emi), tenure:String(l.tenure||'')}; if(type==='accwelcome') return {name:l.name, acno:l.acno||''}; if(type==='launch') return {name:l.name}; if(type==='birthday') return {name:l.name}; return {name:l.name, occasion:occ||''}; };
    ...
    var grMoneyRs=function(l){ var v=grVars(l); if(type==='bounce'||type==='cleared'||type==='chequepresent') return v; var o={},M={amount:1,outstanding:1,arrears:1,emi:1,principal:1,disbursed:1,fine:1}; for(var k in v){ o[k]=(M[k]&&v[k]!=='' && v[k]!=null)?('₹'+v[k]):v[k]; } return o; };
