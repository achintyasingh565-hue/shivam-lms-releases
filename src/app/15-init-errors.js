  ...
  function renderSafetyNudge(){ const el=$('safetyNudge'); if(!el) return; if(!currentUser||currentUser.role!=='admin'){ el.style.display='none'; return; }
    ...
    const badDue=(loans||[]).filter(l=>l && l.due && l.disb && l.due < l.disb); if(badDue.length) msgs.push(badDue.length+' loan record(s) have a <b>Next Due Date before the disbursement date</b> (e.g. '+esc(badDue[0].name||badDue[0].acno||'')+') \u2014 open the record and click \u201c\u21bb use schedule\u201d next to the due date to fix it.');
