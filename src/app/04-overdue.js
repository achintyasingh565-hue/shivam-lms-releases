  ...
  function autoMkItem(l, cat, reason, key){
    ...
    var vars, msg;
    if(cat==='EMI Reminder'){ vars={name:l.name, emi:inrPlain(l.emi), due_date:av('due_date'), acno:l.acno||'', outstanding:inrPlain(l.outstanding), fine:inrPlain(getLateFee()), rate:av('rate'), amount:inrPlain(l.emi), loanid:l.acno||''}; msg=applyVars(TPL.reminder,l,{}); }
    else if(cat==='Overdue Reminder'){ vars={name:l.name, outstanding:inrPlain(l.outstanding), due_date:av('due_date'), fine:inrPlain(getLateFee()), acno:l.acno||'', emi:inrPlain(l.emi), amount:inrPlain(l.outstanding), loanid:l.acno||''}; msg=applyVars(TPL.overdue,l,{}); }
    else if(cat==='Payment Confirmation'){ vars={name:l.name, amount:'', txn:'', acno:l.acno||'', outstanding:inrPlain(l.outstanding)}; msg=applyVars(TPL.thanks,l,{}); }
    else if(cat==='Loan Approval'){ vars={name:l.name, amount:inrPlain(l.principal), loanid:l.acno||'', acno:l.acno||'', emi:inrPlain(l.emi), due_date:av('due_date')}; msg=applyVars(TPL.welcome,l,{}); }
